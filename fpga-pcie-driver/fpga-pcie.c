...
MODULE_DESCRIPTION("Driver for Xilinx generic PCIe demo");
MODULE_AUTHOR("blueardour, Archlab");
MODULE_VERSION("1.00");
MODULE_ALIAS("xilinx_pcie");
MODULE_LICENSE("GPL v2");

#define PCI_DEVICE_ID_GENERIC		0x0007
...
static const char fpga_pcie[] = "fpga_pcie";

static const struct pci_device_id fpga_ids[] = {
	{PCI_DEVICE(PCI_VENDOR_ID_XILINX, PCI_DEVICE_ID_GENERIC)},
	{PCI_DEVICE(PCI_VENDOR_ID_ALTERA, PCI_DEVICE_ID_GENERIC)},
	{PCI_DEVICE(PCI_VENDOR_ID_ACTEL , PCI_DEVICE_ID_GENERIC)},
	{PCI_DEVICE(PCI_VENDOR_ID_LATTICE,PCI_DEVICE_ID_GENERIC)},
	{ /* End: all zeroes */ }
};

static int fpga_pci_direction(int direction)
{
	switch (direction) {
	case DMA_TO_DEVICE:
...
static void fpga_dma_sync_single_for_cpu_pci(struct fpga_endpoint *ep,
